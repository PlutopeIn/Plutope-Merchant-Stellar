...
import {ErrorObject, image, storeDetailProps} from './customizeStoreProps';
...
const customizeStoreController = () => {
  ...
  const {signupDetails} = useAppSelector(
    (state: RootState) => state.userReducer,
  );
  const navigation = useNavigation<AuthNavigationProps>();
  const [storeDetail, setStoreDetail, resetState] =
  ...
  const validation = () => {
    ...
    if (isValid) {
      customizeStoreApiCall();
    }
  };

  const customizeStoreApiCall = async () => {
    // #region Start integrating customizeStore api
    ...
    try {
      ...
      Log('customizeStore response', JSON.stringify(data));
      Snackbar(data?.message);
      dispatch(navigationStep('4'));
      resetState();
      navigation.replace('kybProcess');
    } catch (e: any) {
      setStoreDetail('loading', false);
      Snackbar(e?.data?.message);
      Log('customizeStore error', e);
    }
    // End integrating customizeStore api
  };
