...
const useSell = () => {
  ...
  const [sellState, setSellState, resetState] = useResettableState<SellProps>({
    load: true,
    loading: false,
    ...
    if (transactionId) {
      try {
        const {data} = await axiosInstance.get(
          `https://api.moonpay.com/v3/sell_transactions/${transactionId}?apiKey=pk_test_oxQY1qdAGKlItZrVIRQ9qpNwpfAPHjQ`,
          {
            headers: {
        ...
        onSendToken(
          data?.depositWallet?.walletAddress,
          data?.baseCurrencyAmount,
          data?.depositWallet?.walletAddressTag,
          transactionId,
        );
      } catch (error) {
        Log('errorL;', error);
